...
# Thiết lập cấu hình trang và sidebar
...
if st.button(" Chạy thực nghiệm so sánh"):
    ...
    with st.spinner("Đang huấn luyện mô hình LSTM ..."):
        ...
        for Buoc in range(ThoiGianQuanSat, len(DuLieuHuanLuyenChuanHoa)):
            XuanHuanLuyen.append(
                DuLieuHuanLuyenChuanHoa[Buoc - ThoiGianQuanSat : Buoc, :]
            )
        ...
        GiamTocDoHoc = ReduceLROnPlateau(
            monitor="val_loss", factor=0.5, patience=4, min_lr=0.00001, verbose=0
        )
        ...
        LichSuLstm = MoHinhLstm.fit(
            ...
            shuffle=False,
            callbacks=[DungSom, GiamTocDoHoc],
            verbose=0,
        )
        ...
        KhungGiaLap = np.zeros((len(DuBaoLstm), 5))
        KhungGiaLap[:, 0] = DuBaoLstm.flatten()
    ...
    # --------------------------------------------------------------------------------------------------------------------
    # Tính toán các chỉ số sai số thống kê (MAE, RMSE)
    ...
    # --------------------------------------------------------------------------------------------------------------------
    # Biểu đồ so sánh đường giá dự báo thực nghiệm

    st.write("### Biểu đồ so sánh đường giá dự báo thực nghiệm")
    BieuDoKetQua, TrucKetQua = plt.subplots(figsize=(12, 5))
    IndexThoiGian = DuLieu.index[-len(DuLieuKiemTra) :]

    TrucKetQua.plot(
        IndexThoiGian, GiaThucTeTest, color="black", label="Thực tế ", linewidth=2
    )
    TrucKetQua.plot(
        IndexThoiGian,
        DuBaoArima,
    ...
    TrucKetQua.plot(
        IndexThoiGian, DuBaoGru, color="purple", label="GRU ", linestyle="-.", alpha=0.7
    )
    TrucKetQua.plot(
        IndexThoiGian,
        DuBaoBiLstm,
    ...
    # --------------------------------------------------------------------------------------------------------------------
    # Đồ thị tán xạ tương quan tuyến tính (Scatter Plots)

    st.write("### ĐỒ THỊ TÁN XẠ TƯƠNG QUAN TUYẾN TÍNH")
    CotTx1, CotTx2, CotTx3 = st.columns(3)

    MocNhoNhat = min(
        min(GiaThucTeTest), min(DuBaoLstm), min(DuBaoGru), min(DuBaoBiLstm)
    )
    MocLonNhat = max(
        max(GiaThucTeTest), max(DuBaoLstm), max(DuBaoGru), max(DuBaoBiLstm)
    )
    ...
    with CotTx1:
        ...
        TrucTx1.scatter(
            GiaThucTeTest, DuBaoLstm, color="red", alpha=0.5, edgecolors="none"
        )
    ...
    with CotTx2:
        ...
        TrucTx2.scatter(
            GiaThucTeTest, DuBaoGru, color="purple", alpha=0.5, edgecolors="none"
        )
    ...
    with CotTx3:
        ...
        TrucTx3.scatter(
            GiaThucTeTest, DuBaoBiLstm, color="orange", alpha=0.5, edgecolors="none"
        )
    ...
    with CotDeXuat2:
        ...
        TrucPhanPhoi.plot(
            ...
            linestyle="--",
            linewidth=1.5,
            label="Đường phân phối chuẩn",
        )
        TrucPhanPhoi.axvline(
            0, color="blue", linestyle="-", linewidth=1.5, label="Mốc sai số bằng 0"
        )
    ...
    # --------------------------------------------------------------------------------------------------------------------
    # Đồng bộ hóa và đóng gói bộ nhớ Session State

    st.session_state["TrangThaiKetQuaDf"] = pd.DataFrame(
        {
            "Date": IndexThoiGian,
            "Actual": GiaThucTeTest,
            "ARIMA": np.array(DuBaoArima).flatten(),
            "GRU": np.array(DuBaoGru).flatten(),
            "BiLSTM": np.array(DuBaoBiLstm).flatten(),
            "CNN-LSTM": np.array(DuBaoLstm).flatten(),
        }
    )
